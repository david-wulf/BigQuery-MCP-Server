...
server.registerTool(
  "query",
  {
    description: "Run a SQL query against BigQuery and return results. Only SELECT queries are allowed. A LIMIT clause is automatically added if missing. Claude should use list_datasets, list_tables, and describe_table first to understand the schema before writing queries." + GUARDRAIL_SUFFIX,
    inputSchema: {
...
server.registerTool(
  "gsc_quick_wins",
  {
    description: "Find keywords from GSC bulk export data at positions 4 to 15 with high impressions. These are striking distance keywords that could be pushed to page one. Sorted by traffic opportunity." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
...
server.registerTool(
  ...
  {
    description: "Find pages with high impressions but CTR significantly below the expected benchmark for their ranking position. These are title and meta description optimisation candidates." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool(
  "gsc_site_snapshot",
  {
    description: "Get a quick overview of how the site is performing. Returns total clicks, impressions, CTR, position, unique pages and queries with a comparison to the prior period." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
...
server.registerTool(
  "gsc_content_decay",
  {
    description: "Find pages with consistent traffic decline over three consecutive months from GSC bulk export data. One bad month is noise; three is a problem." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
        device: z.enum(["MOBILE", "DESKTOP", "TABLET"]).optional().describe("Restrict to one device. Omit for all devices, which is the default."),
        country: z.string().optional().describe("Restrict to one country as an ISO-3166-1 alpha-3 code, e.g. deu, aut, che. Omit for all countries, which is the default."),
        dataset: z.string().optional().describe("BigQuery dataset containing GSC data"),
...
server.registerTool(
  "gsc_cannibalisation",
  {
    description: "Find keywords where multiple pages from your site compete against each other. Shows which pages rank for the same query and their respective positions." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool(
  "gsc_topic_cluster",
  {
    description: "See how a group of pages performs as a whole. Aggregates clicks, impressions, CTR, and position for all pages matching a URL path pattern, plus top pages and queries." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
...
server.registerTool(
  "gsc_ctr_benchmark",
  {
    description: "Compare your actual CTR per page against industry benchmarks by position. Flags pages significantly underperforming for their ranking position with verdicts." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool(
  "gsc_report",
  {
    description: "Generate a comprehensive markdown performance report. Covers site snapshot, alerts, quick wins, traffic drops, content decay, and recommendations. Returns the full report as markdown." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool(
  ...
  {
    description: "Analyse anonymous (hidden) query traffic that the GSC API cannot show. Reveals what percentage of your clicks come from queries Google redacts, and which pages get the most hidden traffic. Only possible with BigQuery bulk export." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
        dataset: z.string().optional().describe("BigQuery dataset containing GSC data"),
      },
...
server.registerTool(
  "gsc_seasonal",
  {
    description: "Year-over-year seasonal traffic analysis. Shows monthly clicks, impressions, CTR, and position with YoY comparison. Requires 12+ months of BigQuery data. Impossible with the 16-month rolling GSC API." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
...
server.registerTool(
  "gsc_device_split",
  {
    description: "Find queries where mobile and desktop rank different pages from your site. This device cannibalisation is invisible in the GSC UI and impossible to detect via the API's 3-dimension limit." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool(
  ...
  {
    description: "Classify all your ranking queries by search intent (informational, transactional, commercial, navigational) using regex pattern matching at scale. Shows clicks, impressions, and CTR by intent category." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
        dataset: z.string().optional().describe("BigQuery dataset containing GSC data"),
      },
...
server.registerTool(
  "gsc_ngrams",
  {
    description: "Extract the most common meaningful terms across your entire query set, ranked by clicks. A lightweight alternative to keyword clustering that reveals emerging topics and content themes." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool(
  "gsc_new_keywords",
  {
    description: "Discover queries that appeared in your recent data but were not present in the baseline period. Useful for spotting new ranking opportunities, trending topics, or the impact of recently published content." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX + POSITION_CAVEAT,
    inputSchema: {
...
server.registerTool(
  "gsc_forecast",
  {
    description: "Forecast organic traffic using BigQuery ML ARIMA_PLUS. Trains a time-series model on your historical click data and projects future clicks with confidence intervals. Requires sufficient historical data (ideally 6+ months). This is only possible with BigQuery ML." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
...
server.registerTool(
  "gsc_anomalies",
  {
    description: "Detect traffic anomalies using BigQuery ML. Unlike threshold-based alerts, this understands seasonality and weekly patterns, so it only flags genuinely unexpected traffic changes. Requires sufficient historical data (ideally 6+ months)." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
...
server.registerTool(
  "gsc_discover",
  {
    description: "Google Discover performance from the export: clicks, impressions, CTR, its share of all surfaces, a time series, top URLs, device and country split. Discover is page-based, so nothing groups by query, and its anonymisation is tracked separately via is_anonymized_discover." + GUARDRAIL_SUFFIX + VISUAL_SUFFIX,
    inputSchema: {
        days: z.number().default(28).describe("Number of days to analyse"),
