 ...
 */
let locationMapCache: { raw: string; map: Map<string, string> } | null = null;
 ...
 */
function parseDatasetLocations(raw: string | undefined): Map<string, string> {
  if (!raw) return new Map();
  if (locationMapCache && locationMapCache.raw === raw) return locationMapCache.map;
  ...
  for (const entry of raw.split(",")) {
    ...
    if (sep < 1) {
      throw new Error(
        `Invalid BIGQUERY_DATASET_LOCATIONS entry: "${trimmed}". Expected "dataset:location" or "project.dataset:location".`
      );
    }
    ...
    if (!key || !location) {
      throw new Error(
        `Invalid BIGQUERY_DATASET_LOCATIONS entry: "${trimmed}". Both dataset and location must be non-empty.`
      );
    }
 ...
 */
function lookupDatasetLocation(
 ...
 */
export function extractDatasets(sql: string): string[] {
  ...
  if (list.length > 0 && config.datasetLocations.size > 0) {
    ...
    if (hits.size > 1) {
      const detail = [...hits.entries()]
        ...
        .join(" | ")
      throw new Error(
        `Cross-location query: this statement touches datasets in different BigQuery locations (${detail}). ` +
